...
import com.charles.interview.arena.agent.perception.PerceptionService;
 ...
 * <pre>
 * 0. 感知管线（PerceptionService 7 步：格式校验 → 安全检查 → 规范化 → 意图分类 + 实体提取）
...
public class QuickAskService {
    ...
    public QuickAskResponse quickAsk(Long userId, String query) {
        ...
        // 0. 感知管线（7 步：格式校验 → 安全检查 → 规范化 → 意图分类 + 实体提取）
        PerceptionResult perceptionResult = perceptionService.perceiveText(query);
        if ("BLOCKED".equals(perceptionResult.riskAssessment().level())) {
            log.warn("输入被感知管线拦截: {}", perceptionResult.riskAssessment().reason());
            response.setAnswer("输入被安全检查拦截：" + perceptionResult.riskAssessment().reason());
            return response;
        }

        Intent intent = perceptionResult.intent();
        // 从感知结果提取规范化文本（优先用观察内容，兜底用原始 query）
        String processedText = perceptionResult.observations().isEmpty()
                ? query
                : perceptionResult.observations().get(0).content();
        ...
        if (!reactResult.isSuccess() || reactResult.getFinalAnswer().get("answer") == null) {
            ...
            return fallbackAsk(userId, processedText, intent, response);
        }
    ...
    private String retrieveMemoryContext(Long userId, String query) {
        try {
            // 1. RRF 融合的 query-dependent 记忆（与 query 强相关）
            List<MemoryFragment> fragments = memoryRetriever.retrieveWithRRF(userId, query);
            
            // 2. Query-independent 用户画像摘要（不参与 RRF，直接注入）
            String userProfile = memoryRetriever.retrieveUserProfileSummary(userId);

            if ((fragments == null || fragments.isEmpty()) && userProfile.isBlank()) {
                return "（暂无相关历史学习记忆）";
            }

            StringBuilder sb = new StringBuilder();

            // 相关记忆区域（query-dependent，RRF 融合结果）
            if (fragments != null && !fragments.isEmpty()) {
                for (int i = 0; i < fragments.size(); i++) {
                    MemoryFragment f = fragments.get(i);
                    sb.append("--- 相关记忆 ").append(i + 1)
                      .append("（").append(f.getSource()).append("）---\n")
                      .append(f.getContent()).append("\n\n");
                }
            }

            // 用户画像区域（query-independent，个性化背景）
