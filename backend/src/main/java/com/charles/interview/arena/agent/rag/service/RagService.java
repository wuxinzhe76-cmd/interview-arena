...
public class RagService {
    ...
    public int importQuestionsToVectorStore(){
        ...
        // 先清空 Milvus 旧数据（避免重复导入）
        try {
            vectorStore.delete(questions.stream().map(q -> String.valueOf(q.getId())).toList());
            log.info("已清空 Milvus 旧数据");
        } catch (Exception e) {
            log.warn("清空 Milvus 旧数据失败（可能为空库）：{}", e.getMessage());
        }
        ...
        for (Question question : questions) {
            ...
            // 截断过长内容，避免超过 embedding 模型 token 限制
            int maxLen = 6000;
            if (contentString.length() > maxLen) {
                contentString = contentString.substring(0, maxLen);
            }
    ...
    /**
     * Advanced RAG 流式问答（SSE）
     * <p>
     * 检索阶段同步执行，LLM 生成阶段流式输出 token。
     * SSE 事件流：
     * 1. meta  -> { cacheHit, sourceQuestions }（检索完成后、生成前）
     * 2. token -> LLM 生成的 token 片段（多次发送）
     * 3. done  -> 生成完成
     * 4. error -> 异常消息（出错时）
     */
    public void ragChatStream(String userMessage, SseEmitter emitter) {
        try {
            // 1. 语义缓存查询
            String cached = semanticCache.get(userMessage);
            if (cached != null) {
                log.info("流式 RAG：语义缓存命中");
                sendSseEvent(emitter, "meta", Map.of("cacheHit", true, "sourceQuestions", List.of()));
                // 缓存命中：一次性发送完整答案
                sendSseEvent(emitter, "token", cached);
                sendSseEvent(emitter, "done", "");
                emitter.complete();
                return;
            }

            // 2-5. 同步执行检索链路（查询改写 -> 混合检索 -> Rerank -> 去重重排）
            String rewrittenQuery = queryRewriteTransformer.rewrite(userMessage);
            List<Document> candidates = hybridRetriever.retrieve(rewrittenQuery);
            List<Document> topDocs = rerankService.rerank(rewrittenQuery, candidates, 5);
            topDocs = documentDeduplicator.deduplicate(topDocs);
            ...
            // 发送元数据（引用溯源 + 缓存标记）
            sendSseEvent(emitter, "meta", Map.of("cacheHit", false, "sourceQuestions", sources));

            // 6. 流式生成（Spring AI ChatClient.stream() 返回 Flux<String>）
            ...
            chatClient.prompt()
                    ...
                    .subscribe(
                        ...
                        () -> {
                            // 7. 语义缓存写入
                            semanticCache.put(userMessage, fullAnswer.toString());
                            try {
