...
public class BM25Retriever {
        ...
        public List<Document> retrieve(String query, int topK, String filterExpression) {
                ...
                hits.forEach(hit -> {
                        ...
                        // 过滤 null 值：Spring AI Document 不允许 metadata 包含 null
                        if (esDoc.getQuestionId() != null) metadata.put("questionId", esDoc.getQuestionId());
                        if (esDoc.getTitle() != null) metadata.put("title", esDoc.getTitle());
                        if (esDoc.getCategory() != null) metadata.put("category", esDoc.getCategory());
                        if (esDoc.getDifficulty() != null) metadata.put("difficulty", esDoc.getDifficulty());
                        if (esDoc.getType() != null) metadata.put("type", esDoc.getType());
